...
│   │   mapa_unificado.py, track_gpx.py, tiles_offline.py, consolidar_medios.py,
...
│   │   corregir_timestamp_360.py, generar_galeria_keypoints.py
...
│   │   visualizaciones.md, limpieza_tandas_resultados.md, ideas_externas.md, lecciones_elecciones_td.md,
│   │   inferencia_autor.md, armado_de_tandas.md, plan_keywords.md, gui_fluir.md,
│   │   deploy.md, retorno_fluir_td.md, videos_360_web.md, embeddings_rediseno.md,
│   │   correccion_timestamp_360.md, revision_pre_presentacion.md
├── models/audio/
...
| **CORREGIR 360** | `corregir_timestamp_360.py` | Corrige timestamps de videos 360° Insta360 post-ingesta: `QuickTime:CreateDate` (UTC) → `timestamp_utc` real (CreateDate −3h ART); limpia `ubicacion_video` previo y opcionalmente reubica en GPX (`--reubicar` invoca `ubicar_videos_gpx --mode update --solo-360`); backup automático + `--dry-run`/`--json` | `media` | timestamp_original, timestamp_utc, timezone_note='360_UTC', end_time, updated_at |
| | | Limpia ubicaciones previas | `media_keypoints` | DELETE key=`ubicacion_video` |
| | | | `media_metadata` | DELETE keys `ubicacion_video_estado`, `ubicacion_video_gaps`; reubica vía `ubicar_videos_gpx` si `--reubicar` |
...
| `improve_db.py` | ~22 pasos post-ingesta (colors, keywords, descriptions, combinado, transcribe, keypoints, timestamps, gps, video_metadata, audio_tagging, kw_transcripcion, kw_texto, refinar, geocode, gradiente, weather, dia_semana, astronomia, analizar_video, keypoints_video, keypoints_contexto, limpiar_descripciones). `Todos`/`Manual` cubren todos (overnight, saltea sin red y avisa) | TUI Mejorar DB (7 grupos por tipo de medio: Todos/Manual + Imagenes/Audios/Videos/Textos/Enriquecimiento-Curaduria); CLI `flujos.py improve-db --steps X --mode Y` |
...
| `geocode.py` | Geocodificación inversa (Georef API Argentina, batch) | TUI Mejorar DB 7→5 (Enriquecimiento); también CLI `flujos.py geocode` y `improve-db --steps geocode` |
| `gradiente.py` | Distancia Haversine, elevación, pendiente entre GPS consecutivos | TUI 7→6 (Enriquecimiento); también CLI `flujos.py gradient` y `improve-db --steps gradiente` |
| `astronomia.py` | Posición del sol (NOAA) + clasificación twilight | TUI 7→9 (Enriquecimiento); también CLI `flujos.py astronomia` y `improve-db --steps astronomia` |
...
| `fetch_weather.py` | Clima histórico Open-Meteo ERA5-Land | TUI 7→7 (Enriquecimiento); también `improve-db --steps weather` |
| `dia_semana.py` | Día de la semana desde `timestamp_utc` | TUI 7→8 (Enriquecimiento); también `improve-db --steps dia_semana` |
| `ingest_gpx.py` | Ingesta de track GPX (tracks, waypoints, backfill altitud) | TUI Ingesta→2; CLI `flujos.py ingest-gpx` / `gpx` |
...
| `mapa_ruta.py` | Mapa HTML interactivo (Folium): la línea de ruta usa el **track GPX** (`tracks`, helpers de `track_gpx.py`); los medios quedan como marcadores con su GPS propio. Tiles: **Esri World Light Gray** (`TILE_ESRI_URL`, sin API key; CartoDB deprecado desde 26-ago-2026 — exige key). `--road-colors` colorea segmentos del track por pendiente; `--tolerancia-metros` (default 1000) reporta discrepancias media vs track; `--umbral-gap-aviso` (default 1800) marca en naranja los medios con gap del track ≥ umbral (posición incierta, aviso en el popup). **HTML autocontenido** (assets JS/CSS inline vía `guardar_autocontenido`; los tiles de su vista inicial siguen online). Flags: `--no-markers`, `--output/-o`, `--assets-cache`. Heatmap eliminado | TUI Visualizaciones→1; CLI `flujos.py mapa` |
| `mapas_municipio.py` | Genera un mapa HTML por municipio recorrido, con variantes (`ruta`, `puntos`, `contexto`, `gradiente`). La variante `ruta` resalta el **tramo del track GPX** del municipio sobre el **track completo** de fondo (gris tenue) manteniendo el encuadre en el municipio; `contexto`/`gradiente` usan el tramo/gradiente. La línea de ruta usa helpers de `track_gpx.py`; los medios quedan como marcadores. Nombre: `mapa_municipio_<municipio>_<variante>.html` (slug ASCII: espacios→`_`, **sin acentos** ni símbolos — `'Río Hondo'`→`Rio_Hondo`; convención para evitar problemas de visualización en TouchDesigner). **HTML 100% autocontenido**: tiles de la vista inicial como data URIs (capa única vía `tiles_offline.py`, proveedor **Esri Light Gray** — CartoDB deprecado) + assets JS/CSS de Folium y fuentes de íconos incrustados inline (`guardar_autocontenido`) → el mapa se abre en el Web Render TOP con **CERO red** (el CEF usa cache temporal por TOP, por eso antes re-descargaba los CDN). `--mode skip` genera solo los faltantes en disco, `--mode update` (default) regenera todos, `--no-embebido` deshabilita la incrustación de tiles. Flag `--tolerancia-metros` (default 1000) para reportar discrepancias media vs track; `--umbral-gap-aviso` (default 1800) marca en naranja los medios con gap del track ≥ umbral (posición incierta, aviso en el popup). Otros flags: `--variantes`, `--municipio` (substring), `--zooms`, `--tiles-cache`, `--assets-cache`, `--output/-o`, `--dry-run` | TUI Visualizaciones→1→2; CLI `flujos.py mapa-municipios` / `mapas` |
| `mapa_unificado.py` | Mapa unificado offline/online con clusters multicolor y expansión de transcripción. Offline (default) lee `db/flujos.db` y genera HTML 100% autocontenido (tiles zooms 5-7 + Leaflet.markercluster inline) para `file://` y TouchDesigner; online lee `deploy/db/visualizacion.db` (medios/keypoints) con tiles/markercluster por CDN. Un punto por cada "algo" con GPS (media 2980 + contexto 1908 keypoints + waypoints opcional); sin GPS se ignora; `telegram location` se ignora. Transcripción (`transcription` 4790 segs) se despliega por segmentos en el mismo mapa (1 activo, repliegue Esc/✕/cambio) con polyline punteada. Clusters: monocolor si 1 tipo, `conic-gradient` mixto. Flags: `--modo offline|online`, `--no-contexto`, `--con-waypoints`, `--sin-cluster`, `--sin-segmentos`, `--dry-run`, `--tiles-cache`, `--assets-cache`, `--output/-o` | TUI Visualizaciones→1→3; CLI `flujos.py mapa-unificado` / `unificado` |
| `tiles_offline.py` | Hace los mapas Folium autocontenidos para TouchDesigner: **tiles** de la vista inicial (`tiles_en_bounds`, `zoom_fit_bounds` que replica `fitBounds`, `descargar_tiles_png` con cache en `tiles_cache/esri/` — **versionada por proveedor**, `data_uris`, `js_capa_base_embebida` — JS de capa única con polling) y **assets** JS/CSS/fuentes de Folium + **Leaflet.markercluster** (`descargar_assets` con cache en `assets_cache/`, `inline_fuentes`, `guardar_autocontenido` que reemplaza los tags CDN por contenido inline; fallback CDN si la descarga falla). Proveedor de tiles: **Esri World Light Gray** (`TILE_URL_ESRI`, sin API key; CartoDB deprecado desde 26-ago-2026). Standalone: `python scripts/tiles_offline.py lat0 lon0 lat1 lon1` (tiles) o `python scripts/tiles_offline.py --assets` (assets) | Importado por `mapas_municipio.py`, `mapa_ruta.py` y `mapa_unificado.py` |
...
| `generar_galeria_keypoints.py` | Galerías HTML de keypoints al azar (transcripción/contexto): player + mapa + slideshow 10 fotos cercanas (distancia geo, fallback temporal), Leaflet online, fundido 3s loop infinito. **Portabilidad**: embebe `file://` absolutas vigentes al generar; tras `relocate`/`consolidar` o cambio de computadora **regenerar** (TUI 6→4) | TUI Visualizaciones→4; CLI `flujos.py` no (solo TUI: `python scripts/generar_galeria_keypoints.py --tipo transcripcion|contexto --n 50 [--db PATH] [--output PATH] [--verbose]`) |
| `td/puente_td.py` | Puente BD → TouchDesigner vía OSC (9000→TD, 9001←TD, 9002→TD resultado). Modos: `elecciones` (default) y `fluir` — el modo instalación escucha sin límite de tiempo hasta Enter; en `fluir`, si hay municipios elegidos, emite además el chat de Telegram (`/mensaje` ×N → tabla `fluir_telegram`, criterio web: rango de fechas de los medios, `es_sistema=0`, hora local UTC−3, texto ≤250 chars) y las rutas de mapas por municipio (`/mapa` ×N → tabla `fluir_mapas`, ruta al HTML de `scripts/mapas_municipio.py`) y genera `td/chat_fluir.html` (HTML **autocontenido** del chat — fotos como data URIs, cero red — para el Web Render TOP `web_render_chat`, sincronizado con el loop: el callbacks lo recarga en `/fin` con `?t0=&loop_secs=`) y `td/textos_fluir.html` (HTML **autocontenido** de los textos — `titulo` + `texto_completo`, **uno a la vez con rotación fija de 30 s** en orden de ruta, look markdown `#000`/texto blanco/65ch; si el texto no entra en pantalla hace UN ciclo baja-subir con aire vertical — para el Web Render TOP `web_render_textos`); flags `--no-enviar-telegram`, `--no-enviar-mapas`, `--no-enviar-medios`, `--generar-chat-html`/`--no-generar-chat-html`, `--generar-textos-html`/`--no-generar-textos-html`, `--una-vez`, `--debounce`, `--loop-secs`, `--spec-salida`, `--grupo` | TUI Visualizaciones→3; CLI `python scripts/td/puente_td.py <modo>` |
...
| `ai_media/analyze_video.py` | Análisis de videos con IA: scene detection → ~10 imgs/escena → nitidez → 1 llamada PROMPT_COMBINADO por escena (máx 20 tags); flags `--por-escena`/`--mejores-por-escena` (eliminado `--interval`) | TUI 5→3; también `improve-db --steps analizar_video` / CLI `flujos.py analizar-video` |
| `ai_media/keypoints_video.py` | Keypoints semánticos de video: `media_keypoints` key=`escena`/`keyword`, source='ollama' (desde `video_analysis`); sentinel `keypoints_video_estado` | TUI 5→4; también `improve-db --steps keypoints_video` (post-`analyze_video`): `python scripts/ai_media/keypoints_video.py [--mode]` |
| `keypoints_contexto.py` | Keypoints de contexto (devenir geográfico): F1 interpola track GPX, F2 transiciones (elevación/astronomía/movimiento), F3 georef+clima con cache, F4 escribe `contexto_*` | TUI Hoja 3→2 / 5→5; también `improve-db --steps keypoints_contexto`: `python scripts/keypoints_contexto.py [--mode]` |
...
| `corregir_timestamp_360.py` | Corrige timestamps de videos 360° Insta360 post-ingesta: `QuickTime:CreateDate` (UTC) → `timestamp_utc` real (CreateDate −3h ART); limpia `ubicacion_video` previo y opcionalmente reubica en GPX (`--reubicar` invoca `ubicar_videos_gpx --mode update --solo-360`); backup automático + `--dry-run`/`--json` | TUI `7) Scripts temporarios` → 1; CLI `flujos.py corregir-360` / `corregir360` `--mode skip|update|replace --dry-run --reubicar --json --verbose` |
...
| `ai_media/refinar_keywords.py` | Refina/unifica keywords (léxico + diccionario de sinónimos) | TUI 6→2 / 7→1; también `improve-db --steps refinar` |
| `ai_media/keywords_transcripciones.py` | Keywords del SENTIDO desde transcripciones (`--origen transcripcion`, default → `ia_keywords_transcripcion`) o desde textos .md ingresados (`--origen texto` → `ia_keywords_texto`) | TUI 4→4 / 6→1; también `improve-db --steps kw_transcripcion/kw_texto` |
| `ai_media/audio_tagging.py` | Sonidos ambientales (sherpa-onnx CED-mini, local) | TUI 4→3 / 5→2; también `improve-db --steps audio_tagging` |
...
- Tablas de datos del retorno "Fluir" (canal 9002; se crean con `td/crear_tablas_fluir.dat` y las llena `td/fluir_callbacks.dat` desde `/flujos/fluir/*`): `fluir_estado` (clave-valor), `fluir_fotos` / `fluir_videos` / `fluir_sonidos` (medios por tipo, columnas `[media_id, ruta, keypoint, hora, tipo]`), `fluir_textos` (textos; agrega `titulo` + `texto` = contenido real del texto como unidad de medio, vía `/flujos/fluir/texto`), **`fluir_videos_360`** (videos 360° separados del resto — marker `media.subtype='360'`, escrito por `improve_db --step video_metadata`), `fluir_chiches` (hora, texto, lat, lon, municipio, provincia, departamento — v2.3 con variantes pesadas y sostenimiento), **`fluir_telegram`** (chat de Telegram de los municipios elegidos: `[id, from_name, texto, hora, fecha, tipo, fotos, municipio]`, vía `/flujos/fluir/mensaje`; solo si hay municipios, criterio web = rango de fechas de los medios, `es_sistema=0`, hora local UTC−3) y **`fluir_mapas`** (rutas de mapas por municipio: `[municipio, ruta]`, vía `/flujos/fluir/mapa`; solo si hay municipios, la ruta apunta al HTML generado por `scripts/mapas_municipio.py`)
- **`web_render_chat`** — Web Render TOP (manual, como `osc_in2`): apunta a `td/chat_fluir.html` (HTML **autocontenido** del chat que genera `puente_td.py --generar-chat-html`); el callbacks lo recarga en cada `/fin` con `?t0=&loop_secs=` (chat sincronizado con la hora del loop; ver `docs/retorno_fluir_td.md`)
- **`web_render_textos`** — Web Render TOP (manual, como `osc_in2`): apunta a `td/textos_fluir.html` (HTML **autocontenido** de los textos que genera `puente_td.py --generar-textos-html`); el callbacks lo recarga en cada `/fin` con `?t0=&loop_secs=` (rotación fija de 30 s por texto en orden de ruta, independiente del largo del loop; si no entra en pantalla hace un único ciclo baja-subir que incluye el aire vertical; ver `docs/retorno_fluir_td.md`)
...
### 3. Doble caché al cambiar plantillas HTML del puente (`puente_td.py`)

Los HTML autocontenidos que el puente escribe para TD (`td/textos_fluir.html`, `td/chat_fluir.html`) pasan por **dos cachés independientes**; si solo se refresca una, el cambio parece "no aplicarse":

| Capa | Síntoma | Solución |
|---|---|---|
| Proceso Python del puente | Cada ráfaga reescribe el HTML con **datos frescos pero plantilla vieja** (los módulos se importan una sola vez al arrancar el proceso) | Reiniciar `flujos.py` / el proceso `puente_td.py` |
| Web Render TOP (CEF) | El operador sigue mostrando la página anterior aunque `par.url` cambie; **el cache-buster `?t0=` NO es confiable sobre `file://`** | Recargar/reiniciar el operador Web Render TOP |

Checklist tras tocar cualquier plantilla HTML dentro de `puente_td.py`: reiniciar puente → re-tirar ráfaga → verificar mtime y contenido de `td/*.html` → recargar los Web Render TOP en TD.
