...
- **`web_render_textos`** — Web Render TOP (manual, como `osc_in2`): apunta a `td/textos_fluir.html` (HTML **autocontenido** de los textos que genera `puente_td.py --generar-textos-html`); el callbacks lo recarga en cada `/fin` con `?t0=&loop_secs=` (textos rotando uno a la vez con la hora del loop; ver `docs/retorno_fluir_td.md`)
