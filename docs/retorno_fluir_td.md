...
| 15 | Filtro duro por tags en el loop | **Resuelta** | las tags elegidas en TD ahora FILTRAN (no solo prioridad): `_filtrar_media` agrega `EXISTS` con OR de `LIKE '%tag%'` sobre las 5 fuentes de keywords (`ia_keywords`, `ia_keywords_transcripcion`, `ia_keywords_texto`, `ia_keywords_sonido`, `ia_keywords_video` — el mismo universo de `elec_tags`). Un medio pasa si contiene ALGUNA tag. **Fallback**: si el arco queda con menos de `MIN_MEDIOS_FALLBACK_TAGS` (default 1), se re-genera sin tags (prioridad, nunca vacío) y el resumen anota `FALLBACK a prioridad`. **Pendiente**: reemplazar el fallback por aviso real de "no hay suficientes medios seleccionados" |
